...
from .metrics import TokenCls
from .modules import SMARTDecoder
...
def joint_scene_from_states(states, object_ids) -> sim_agents_submission_pb2.JointScene:
    if sim_agents_submission_pb2 is None:
        raise ImportError("waymo_open_dataset is required for Waymo submission export")
    states = states.numpy()
...
class SMART(pl.LightningModule):
    ...
    def __init__(self, model_config) -> None:
        ...
        self.map_token_traj_path = self._resolve_token_path(
            getattr(model_config, "map_token_path", None),
            "map_traj_token5.pkl",
        )
        self.init_map_token()
        self.token_path = self._resolve_token_path(
            getattr(model_config, "agent_token_path", None),
            f"cluster_frame_5_{model_config.decoder.token_size}.pkl",
        )
    ...
    def _resolve_token_path(self, override_path, filename: str) -> str:
        if override_path is not None:
            candidate = Path(override_path)
            if candidate.is_file():
                return str(candidate)

        local_candidate = Path(__file__).resolve().parent / "tokens" / filename
    ...
    def training_step(self,
        ...
        self.log('train/cls_loss', cls_loss, prog_bar=False, on_step=True, on_epoch=True, batch_size=1)
        return loss
    ...
    def test_step(self, data, batch_idx):
        ...
        next_token_eval_mask = pred['next_token_eval_mask']
        cls_loss = self.cls_loss(next_token_prob[next_token_eval_mask], next_token_idx_gt[next_token_eval_mask])
        self.log('test/loss', cls_loss, prog_bar=True, on_step=False, on_epoch=True, batch_size=1)
        ...
        valid_mask = data['agent']['valid_mask'][:, self.num_historical_steps:]
        eval_mask = data['agent']['valid_mask'][:, self.num_historical_steps - 1]
        self.minADE.update(pred=rollout['pred_traj'][eval_mask], target=rollout['gt'][eval_mask], valid_mask=valid_mask[eval_mask])
        self.minFDE.update(pred=rollout['pred_traj'][eval_mask], target=rollout['gt'][eval_mask], valid_mask=valid_mask[eval_mask])
        self.log('test/minADE', self.minADE, prog_bar=False, on_step=False, on_epoch=True, batch_size=1)
        self.log('test/minFDE', self.minFDE, prog_bar=False, on_step=False, on_epoch=True, batch_size=1)
